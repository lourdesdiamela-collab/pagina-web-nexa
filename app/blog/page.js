...
export default function LearnPage() {
  ...
  return (
    <>
      <Navbar />
      <main style={{ paddingTop: 'clamp(88px, 10vw, 120px)', background: '#f5f0fa', minHeight: '100vh' }}>
        <section className="container" style={{ paddingBottom: 80 }}>
          <header style={{ textAlign: 'center', marginBottom: 22 }}>
            <span className="section-tag">Blog NEXA</span>
            ...
            {filtered.map((article) => (
              <article key={article.slug} style={{ borderRadius: 16, border: '1px solid rgba(16,18,34,0.12)', background: 'white', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: 14, display: 'grid', gap: 10, flex: 1 }}>
                  ...
                  <Link href={`/blog/${article.slug}`} style={{ marginTop: 'auto', display: 'inline-flex', alignItems: 'center', gap: 6, color: '#5530c6', fontWeight: 700, textDecoration: 'none' }}>
