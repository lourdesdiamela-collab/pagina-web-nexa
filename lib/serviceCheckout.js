...
import { getServicePlan } from './servicePlans.mjs';
...
/*
 * Valida los datos de contacto y resuelve el plan contra la lista de precios
 * del repositorio (lib/servicePlans.mjs).
 *
 * IMPORTANTE: el monto NO se acepta del cliente. Antes llegaba por el body
 * (puesto ahí desde la URL `?amount=...`) y solo se validaba que fuera mayor a
 * cero, así que se podía contratar el plan de $549.900 por un peso. Ahora el
 * cliente manda un identificador de plan y el precio sale de la lista. Si el
 * identificador no existe, se rechaza el pedido.
 *
 * Devuelve { error } o { error: null, plan, contacto }.
 */
export function resolveServiceCheckout(body) {
  const required = ['name', 'email', 'phone'];
  const missing = required.filter((f) => !String(body?.[f] || '').trim());
  if (missing.length > 0) {
    return { error: `Faltan campos obligatorios: ${missing.join(', ')}.`, plan: null, contacto: null };
  }

  const plan = getServicePlan(body?.plan);
  if (!plan) {
    return { error: 'El plan seleccionado no existe. Volvé a Servicios y elegí un plan.', plan: null, contacto: null };
  }
...
export async function sendServiceLeadEmails({ reference, name, email, phone, company, servicio, planLabel, amount, billing, method, termsAcceptance }) {
  const methodLabel = method === 'transfer' ? 'Transferencia bancaria' : 'Mercado Pago';

  // La aceptación de los T&C también se manda en el mail interno: mientras el
  // CRM no tenga dónde guardarla (ver informe), el mail es el único registro
  // duradero de quién aceptó, cuándo y desde qué IP.
  const termsRow = termsAcceptance
    ? `<tr><td><strong>Aceptó T&amp;C</strong></td><td>${new Date(termsAcceptance.termsAcceptedAt).toLocaleString('es-AR', { timeZone: 'America/Argentina/Buenos_Aires' })} (hora Argentina) — IP ${termsAcceptance.termsAcceptedIp || 'no disponible'} — versión ${termsAcceptance.termsVersion}</td></tr>`
    : '';
