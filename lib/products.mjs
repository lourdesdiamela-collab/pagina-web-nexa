...
// Se eliminó el helper sample(): solo lo usaba el generador de reseñas falsas.
...
/*
 * NOTA (septiembre 2026): acá vivían NAMES_POOL y COMMENTS_POOL, dos listas
 * con las que buildProduct() fabricaba, para CADA UNO de los ~100 recursos del
 * catálogo, una valoración falsa (rating entre 4.2 y 5.0), una cantidad falsa
 * de reseñas (entre 9 y 142) y tres reseñas firmadas con nombres inventados
 * ("Martina G.", "Facundo R.", "Sofía L.", etc.) y comentarios de una lista
 * fija ("Justo lo que necesitaba, clarísimo y fácil de aplicar.", "Las
 * plantillas solas ya valen la compra.", etc.).
 *
 * El seed las cargaba en la base (3 por producto), y como el rating que se
 * muestra sale del promedio real de esas 3 filas, todos los productos
 * terminaban mostrando la misma valoración idéntica: "4.7 (3)".
 *
 * Ahora los productos nacen sin valoración ni reseñas. Cuando haya compradores
 * de verdad, las reseñas van a salir solas de la tabla Review.
 */
...
function buildProduct(category, title, subtitle, indexInCategory, globalIndex) {
  ...
  // Sin reseñas reales no hay valoración: 0 reseñas, rating 0. La UI oculta
  // las estrellas cuando reviewsCount es 0 (ver components/aprende/StarRating).
  const rating = 0;
...
/*
 * "Más vendido": ningún producto nace con este flag. Antes se repartía entre
 * 12 productos ordenando por rating * reviewsCount, o sea por las reseñas
 * inventadas — el badge "Más vendido" aparecía sin que se hubiera vendido
 * nada. Cuando haya ventas reales, Lu puede marcar los que quiera desde el
 * panel admin (/aprende/admin/productos), que escribe el flag en la base.
 *
 * "Destacado" sí se sigue asignando: es una selección editorial (un recurso
 * por categoría, el primero de cada una), no una afirmación sobre ventas ni
 * sobre valoraciones de clientes.
 */
const FEATURED_SLUGS = new Set(
  CATEGORIES.map((cat) => PRODUCTS.filter((p) => p.category === cat.slug)[0]?.slug).filter(Boolean)
);
...
// Devuelve solo los productos marcados a mano como "más vendido" desde el
// panel admin. Mientras no haya ninguno marcado, devuelve lista vacía y la
// fila correspondiente no se muestra.
