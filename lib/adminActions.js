...
import { altaAutomaticaPorPago, sincronizarArrepentimiento } from '@/lib/crmSync';
...
/* ── Reintentar alta automática en el CRM ── */
// Botón "Reintentar sync" de /aprende/admin/leads, para leads de checkout de
// servicios que quedaron sin sincronizar (el CRM no respondió, la variable
// SITE_TO_CRM_SECRET no estaba cargada, etc.). Usa los datos que ya quedaron
// guardados en el Lead -- no depende de que Mercado Pago vuelva a avisar.
export async function resyncLeadToCrm(formData) {
  await requireAdminAction();
  const id = String(formData.get('id') || '');
  ...
  if (!lead.mpPaymentId) {
    // Leads del formulario de contacto (no vienen de un pago) usan la
    // sincronización de /api/contact, no esta -- acá no hay nada para
    // reintentar sin un mpPaymentId.
    revalidatePath('/aprende/admin/leads');
    return;
  }

  const plan = lead.planId ? getServicePlan(lead.planId) : null;

  const sync = await altaAutomaticaPorPago({
    mpPaymentId: lead.mpPaymentId,
    reference: lead.reference,
    lineSlug: lead.service,
    planId: lead.planId,
    planLabel: plan?.planLabel ?? lead.planId,
    monto: lead.amount || 0,
    nombre: lead.name,
    email: lead.email,
    telefono: lead.phone,
...
/* ── Reintentar sincronización de arrepentimiento en el CRM ── */
// Botón "Reintentar sync" de /aprende/admin/arrepentimiento, para casos que
// quedaron sin sincronizar (el CRM no respondió, la variable
// SITE_TO_CRM_SECRET no estaba cargada, etc.). Usa los datos que ya quedaron
// guardados acá -- no depende de que el visitante vuelva a mandar el formulario.
export async function resyncArrepentimientoToCrm(formData) {
  await requireAdminAction();
  const id = String(formData.get('id') || '');
  const caso = await prisma.arrepentimientoRequest.findUnique({ where: { id } });
  if (!caso) return;

  const sync = await sincronizarArrepentimiento({
    numero: caso.numero,
    nombre: caso.nombre,
    email: caso.email,
    telefono: caso.telefono,
    identificacionCompra: caso.identificacionCompra,
    motivo: caso.motivo,
  });
  ...
  revalidatePath('/aprende/admin/arrepentimiento');
}
